...
class TSP(gym.Env):
    def __init__(
        ...
        cum_reward: Optional[bool] = False,
    ...
    def scaling(self):
        x_min, x_max, x_avg, x_std = self.x.min(), self.x.max(), self.x.mean(), self.x.std()
        y_min, y_max, y_avg, y_std = self.y.min(), self.y.max(), self.y.mean(), self.y.std()
        self.x_unit = (x_max - x_min) / (self.n_stops * 10)
        self.y_unit = (y_max - y_min) / (self.n_stops * 10)

        x_norm = (self.x - x_avg)/x_std
        y_norm = (self.y - y_avg)/y_std

        self.data_norm = np.stack([x_norm, y_norm], axis = 1)
...
if __name__ == '__main__':
    try:
        ...
        print('Trouble importing package!')
    
    path = os.getcwd()    
    data = pd.read_csv(path + r'\custom_gym_torch_3\trimmed_data.csv')
    data.drop_duplicates(subset = data.columns[-2:], keep = 'first', inplace = True)
    data = data[['OUTLET_LATITUDE', 'OUTLET_LONGITUDE']].head(100).to_numpy()

    env = TSP(data = data, fixed_start = True, cum_reward = False, render_mode = None)
